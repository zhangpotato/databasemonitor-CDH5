...
public class PushMetrics {
    public static HashMap<String, Integer> serviceNumber = null;
    public static HashMap<String, Integer> clusterStatus = null;
    ...
    public static void sendLongDBMetrics() throws IOException {
        ...
        serviceNumber = new HashMap<>();
        clusterStatus = new HashMap<>();

        serviceNumber = HBaseStatus.getServiceStatus();
        clusterStatus = ClusterStatus.getCurrentClusterStatus();
    ...
    public static void sendHBaseMetrics() throws IOException {
        ...
        for (Map.Entry<String, ArrayList> entry : regionServersInfo.entrySet()) {
            String version = entry.getValue().get(1).toString();
            guage.labels("longdb-hbase-RequestsPerSecond", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(2).toString()));
            guage.labels("longdb-hbase-NumRegions", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(3).toString()));
            guage.labels("longdb-hbase-UsedHeap", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(4).toString()));
            guage.labels("longdb-hbase-MaxHeap", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(5).toString()));
            guage.labels("longdb-hbase-MemstoreSize", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(6).toString()));
            guage.labels("longdb-hbase-RequestPerSecond", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(7).toString()));
            guage.labels("longdb-hbase-ReadRequestCount", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(8).toString()));
            guage.labels("longdb-hbase-WriteRequestCount", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(9).toString()));
            guage.labels("longdb-hbase-NumStores", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(10).toString()));
            guage.labels("longdb-hbase-NumStorefiles", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(11).toString()));
            guage.labels("longdb-hbase-StorefileSizeUncompressed", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(12).toString()));
            guage.labels("longdb-hbase-StorefileSize", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(13).toString()));
            guage.labels("longdb-hbase-IndexSize", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(14).toString()));
            guage.labels("longdb-hbase-BloomSize", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(15).toString()));
            guage.labels("longdb-hbase-NumCompactingKVs", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(16).toString()));
            guage.labels("longdb-hbase-NumCompactedKVs", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(17).toString()));
            guage.labels("longdb-hbase-RemainingKVs", version, entry.getKey()).set(Double.parseDouble(entry.getValue().get(18).toString()));
        }
        guage.register(registry);
        groupingKey.put("instance", "my_instance");
        pg.pushAdd(registry, "pushgateway", groupingKey);
